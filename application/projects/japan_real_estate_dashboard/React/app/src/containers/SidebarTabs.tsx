...
const SidebarTabs: React.FC = () => {
    ...
    const onTabClicked = (event: any) => {
        ...
        if (event.target.id === "Sidebar_tab_regions" 
            ...
            regionsTab?.add("show");
            regionsIcon?.add("show");
            optionsTab?.add("hide");
            optionsIcon?.add("hide");
            optionsPage.add("hide");
            regionsPage.add("show");
        }

        if (event.target.id === "Sidebar_tab_options" 
            || event.target.id === "Sidebar_tab_options_icon") {
            optionsTab?.remove("hide");
            optionsIcon?.remove("hide");
            regionsTab?.remove("show");
            regionsIcon?.remove("show");
            regionsPage.remove("show");
            optionsPage.remove("hide");
        }
    }
    ...
    return (
        <form className="Sidebar_tab">
            ...
            <button id="Sidebar_tab_options"
                ...
                <label htmlFor="optionSliders"></label>

                <svg xmlns="http://www.w3.org/2000/svg" name="optionSliders" className="ionicon" id="Sidebar_tab_options_icon" width="100%" viewBox="0 0 512 512">
                    ...
                    <circle cx="176" cy="256" r="32" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32"/>
                    <circle cx="336" cy="384" r="32" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="32"/>
                </svg>

            </button>

            <button id="Sidebar_tab_regions"
                    onClick={onTabClicked}>
                <label htmlFor="regionSelectors"></label>

                <svg xmlns="http://www.w3.org/2000/svg" name="regionSelectors" className="ionicon" id="Sidebar_tab_regions_icon" width="100%" viewBox="0 0 512 512">
